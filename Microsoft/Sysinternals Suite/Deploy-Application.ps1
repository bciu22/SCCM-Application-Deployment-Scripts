...
Try {
	...
	[string]$appName = 'Sysinternals Suite'
	[string]$appVersion = ''
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '20160106'
	...
	If ($deploymentType -ine 'Uninstall') {
        ...
        Copy-File -Path "$dirSupportFiles\SysinternalsSuite" -Destination "$ENV:ProgramFiles" -Recurse
        If(!($ENV:Path -contains "$ENV:ProgramFiles\SysinternalsSuite\"))
        {
            Write-Log -Message "PATH not found, updating"
            $oldPath = (Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\Session Manager\Environment' -Name PATH).Path
            Write-Host $oldPath
            $newPath = $oldPath + ";$ENV:ProgramFiles\SysinternalsSuite\"
            Write-Host $newPath
            Set-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control\Session Manager\Environment' -Name PATH –Value $newPath
        }
        Else
        {
            Write-Log -Message "PATH already exists, not updating"
        }
