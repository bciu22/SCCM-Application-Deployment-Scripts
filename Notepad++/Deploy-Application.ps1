...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Execute-Process -Path 'npp.6.8.6.Installer.exe' -Parameters '/S'
		...
		#region edit the config file do not do auto brackets!!! Arggghhhhh
		$ConfigFile="C:\Program Files (x86)\Notepad++\config.model.xml"
		$o=[xml](Get-Content  $ConfigFile)
		$ai = $o.NotepadPlus.GUIConfigs.ChildNodes | ?{$_.name -eq "auto-insert"}
		$ai.parentheses="no"
		$ai.brackets="no"
		$ai.curlyBrackets="no"
		$nu=$o.NotepadPlus.GUIConfigs.ChildNodes | ?{$_.name -eq "noUpdate"}
		$nu.'#text'="no"
		$o.Save($ConfigFile)
		#endregion
